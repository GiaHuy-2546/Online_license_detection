...
# LOAD YOLO MODEL
net = cv2.dnn.readNetFromONNX('D:\Project/yolov5/runs/train/Model/weights/best.onnx')
...
def save_text(filename, text):
    ...
    with open('{}\WebbApp2\static\predict/{}.txt'.format(folder_Project, name), mode='w') as f:
        f.write(text)
    f.close()
...
def detect_license_plate(pathsave, filename):
    ...
    image = yolo_predictions(img,net, filename)
    return image
    
cap = cv2.VideoCapture('D:\CodingThings\Downloading things/TEST.mov')
